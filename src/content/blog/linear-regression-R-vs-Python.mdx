...
Let's start with an experiment.  We have ten features, and the tenth is
almost a duplicate of the ninth.  This situation could easily arise when
dealing with data sets with many features.  In this experiment, we take
the case to the extreme to make a point.  In the training data the two columns
agree to about one part in $10^8$.  At
test time the redundancy has drifted ever so slightly, to about one part in $10^5$.
The data generating process itself is perfectly well behaved in
the sense that the true $\beta$ and the noise are both at a moderate
level.
...
We fit the same model in R, on the exact same data (exported to CSV with
...
Three implementations of the most classical model in statistics, one
dataset.  Here is what comes back:
...
Now the coefficients, where things get even more interesting.
...
| ------------ | --------- | --------- | --------- | ------------ |
| Truth        | 0         | 3         | 1.5       | 0.5          |
| R lm         | −0.013    | 3.011     | 1.925     | NA           |
| scikit-learn | −0.013    | 3.011     | 0.962     | 0.962        |
| statsmodels  | −0.016    | 3.016     | $+5.6 \times 10^6$ | $-5.6 \times 10^6$ |

![Coefficient comparison on a symlog scale](../../assets/linreg/coefficients.png)

R refuses to estimate $\beta_{10}$ at all: it reports `NA`, prints
`(1 not defined because of singularities)` in the summary, and produces a
...
disagree at the scale of $10^{-5}$ at test time, a coefficient of $5.6$
...
Armed with the two approaches, we can now name exactly what each library
...
multiple places: the `NA` in the coefficient table and the rank-deficiency warning at
